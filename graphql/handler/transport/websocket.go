...
func (c *wsConnection) init() bool {
	...
	case initMessageType:
		if len(m.payload) > 0 {
			...
			err := sonic.ConfigStd.Unmarshal(m.payload, &c.initPayload)
			if err != nil {
				return false
			}
		}
		...
		if initAckPayload != nil {
			initJsonAckPayload, err := sonic.ConfigStd.Marshal(*initAckPayload)
			if err != nil {
				panic(err)
			}
...
func (c *wsConnection) sendError(id string, errors ...*gqlerror.Error) {
	...
	b, err := sonic.ConfigStd.Marshal(errs)
	if err != nil {
		panic(err)
	}
...
func (c *wsConnection) sendConnectionError(format string, args ...any) {
	b, err := sonic.ConfigStd.Marshal(&gqlerror.Error{Message: fmt.Sprintf(format, args...)})
	if err != nil {
		panic(err)
	}
